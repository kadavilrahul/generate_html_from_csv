...
let templatePath = './product.ejs'; // Default template, can be changed
let useChatbot = false; // Option to use chatbot version
let baseUrl = 'https://your_website.com'; // Replace with your base URL
let dataDir = './data'; // Will be updated based on domain
let cacheFilePath = path.join(dataDir, 'generation_cache.json'); // Will be updated based on domain
...
function ensureOutputDir(folderLocation, cb) {
  ...
  // Update dataDir to be domain-specific
  dataDir = `./data_${domain}`;
  cacheFilePath = path.join(dataDir, 'generation_cache.json');
...
// Function to select template interactively
async function selectTemplate() {
  return new Promise((resolve, reject) => {
    ...
    rl.question('\nEnter template choice (1, 2, 3, or press Enter for modular): ', (answer) => {
      ...
      if (answer === '2') {
        console.log('Selected: Chatbot template (product_with_chatbot.ejs)');
...
function loadDbCredentials(folderLocation) {
  const domain = folderLocation.split('/').pop();
  const domainDataDir = `./data_${domain}`;
  const domainCredentialsFile = `${domainDataDir}/${domain}_database_credentials.conf`;
      ...
      for (const product of jsonObj) {
        try {
          ...
          if (fs.existsSync(configPath)) {
            ...
            console.warn('config.json not found, using default template data');
          }
  ...
  try {
    ...
    if (csvFileArg) {
      if (fs.existsSync(csvFileArg)) {
        ...
        // Show template selection first, then CSV selection
        if (!useChatbot) {
      ...
      // Interactive mode - show template selection first, then CSV selection
      if (!useChatbot) {
        await selectTemplate();
      }
      selectedCsvPath = await selectCSVFile();
